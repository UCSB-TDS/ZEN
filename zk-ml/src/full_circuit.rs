...
impl ConstraintSynthesizer<Fq> for FullCircuitOpLv3Pedersen {
    fn generate_constraints(self, cs: ConstraintSystemRef<Fq>) -> Result<(), SynthesisError> {
        ...
        //println!("within circuit l1 output {:?}\n", y);

        let mut y_out = y.clone();
        relu_u8(&mut y_out, self.y_0);

        let l1_circuit = FCCircuitU8BitDecomposeOptimizedSIMD {
            x: self.x,
            l1_mat: self.l1,
            y: y.clone(),
            remainder: remainder1.clone(),
        ...
        //println!("within circuit l2 output {:?}\n", zz);

        let l2_circuit = FCCircuitU8BitDecomposeOptimizedSIMD {
            x: y_out,
            l1_mat: self.l2,
            y: self.z,
            remainder: remainder2.clone(),
