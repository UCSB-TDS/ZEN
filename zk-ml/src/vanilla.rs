...
#[allow(non_snake_case)]
pub(crate) fn relu_u8(input: &mut [u8], zero_point: u8) {
    for e in input {
        if *e < zero_point {
            *e = zero_point
        }
...
pub(crate) fn relu4d_u8(input: &mut Vec<Vec<Vec<Vec<u8>>>>, zero_point: u8) {
    for i in 0..input.len() {
        for j in 0..input[i].len() {
            for k in 0..input[i][j].len() {
                for l in 0..input[i][j][k].len() {
...
pub(crate) fn vec_conv_u8(
    ...
    //println!("x {:?}, x_0 {}", vec[0], vec_0);
    //println!("kernel {:?}, kernel_0 {}\n\n", kernel[0].clone(), kernel_0);

    for n in 0..batch_size {
        for h in 0..(input_height - kernel_size + 1) {
            for w in 0..(input_width - kernel_size + 1) {
                for k in 0..num_kernels {
                    let tmp = conv_kernel_scala_u8(&vec[n], &kernel[k], h, w, vec_0, kernel_0);
                    res[n][k][h][w] = ((tmp as u64
                        * (multiplier[k] as f64 * (2u64.pow(22)) as f64) as u64)
                        / 2u64.pow(22)) as u8
                        + res_0;

                    /*
                    NOTE : the Rust code is equivalent to the following numpy code:
...
pub(crate) fn avg_pool_scala_u8(
    ...
    let mut output = vec![vec![vec![vec![0u8; input_width / kernel_size]; input_height / kernel_size]; num_channels]; batch_size];
    for n in 0..batch_size {
...
pub(crate) fn avg_pool_with_remainder_scala_u8(
    ...
    let input_height = vec[0][0].len(); // height of image
    let input_width = vec[0][0][0].len(); // width of image
    let mut output = vec![vec![vec![vec![0u8; input_width / 2]; input_height / 2]; num_channels]; batch_size];
    let mut remainder = vec![vec![vec![vec![0u8; input_width]; input_height]; num_channels]; batch_size];
...
pub fn full_circuit_forward_u8(
    ...
    println!("l2 output {:?}\n", z);
    println!("y_0 {}, l2_mat_0 {}, l2_output_0 {}", y_0, l2_mat_0, z_0);
...
pub(crate) fn avg_pool_scala(
    ...
    let input_height = vec[0][0].len(); // height of image
    let input_width = vec[0][0][0].len(); // width of image
    let mut output = vec![vec![vec![vec![0i8; input_width / kernel_size]; input_height / kernel_size]; num_channels]; batch_size];
    for n in 0..batch_size {
        for c in 0..num_channels {
            for h in 0..(input_height / kernel_size) {
                for w in 0..(input_width / kernel_size) {
